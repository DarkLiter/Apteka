...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Получение следующего доступного ID клиента
    $nextIdQuery = "SELECT MAX(id_client) AS maxId FROM Клиенты";
    $result = $conn->query($nextIdQuery);
    $row = $result->fetch_assoc();
    $nextId = $row["maxId"] + 1;

    // Подготовленный запрос для добавления новой записи
    $stmt = $conn->prepare("INSERT INTO Клиенты (id_client, first_name, last_name, surname, number) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("issss", $nextId, $first_name, $last_name, $surname, $number);
...
<html>
<head>
    <title>Аптека - Добавление нового клиента</title>
</head>
<body>
    <h1>Аптека - Добавление нового клиента</h1>
